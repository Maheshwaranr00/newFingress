...
let expansion : Locator[],countOfChart : any;
let yearMonth: string | null, day : string | null;
//let count: string | null,no1: Number; last scripts variables
...
test.beforeEach(async()=>{
     ...
     await test.step('Landed on the master details page from the home page',async()=>{
...
test('Chart - Expand & close options',async()=>{        
    await test.step('fetching the locators of all expansion option',async()=>{
        await page.locator('i[class*="expand"]').nth(2).waitFor({state:"visible"});
        expansion = await page.locator('i[class*="expand"]').all();
        countOfChart = expansion.length;
    })    
    for(let i=0;i<countOfChart;i++){
        await test.step('clicking on the expansion',async()=>{
            await expansion[i].click();
        })
        await test.step('download should be visible',async()=>{
            await expect(page.locator('text="Download"')).toBeVisible();
        })
        await test.step('clicking on the close button',async()=>{
...
test('Verify the page is navigated to detail form and vice versa by discarding it',async()=>{    
    await test.step('clicking on the dashboard option',async()=>{
        await page.locator('text="Invoice # INV50967 raised on 2023-08-22T00:00:00.000Z "').click();
    })    
...
test('Verify the operation of check boxes selection and search box in detail form',async()=>{        
   await test.step('Clicking on the dashboard option',async()=>{
    ...
    await test.step('Output and search option should be hidden',async()=>{
    ...
    await test.step('getting displayed date from due date and comparing that with selected date in console',async()=>{
...
test('Verify the selection with radio buttons ',async()=>{        
   await test.step('clicking on the dashboard option',async()=>{
        await page.locator('text="Invoice # INV50967 raised on 2023-08-22T00:00:00.000Z "').click();
    })
    for(let i=0;i<3;i++){
